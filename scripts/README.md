...
This directory contains portable, self-contained scripts for implementing git
quality standards in any project.
...
1. **Project Code**: Change the project code pattern
   ...
   BRANCH_REGEX="^(feature|fix|...)/epic-[A-Z]{3}-[0-9]{4}-.+"
...
3. **Branch Convention**: Customize project code pattern
   ...
   BRANCH_REGEX="^(feature|fix|...)/epic-[A-Z]{2,4}-[0-9]{4}-.+"
...
All scripts support `.validation_exceptions` file for excluding files:
